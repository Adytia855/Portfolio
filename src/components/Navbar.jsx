      ...
      <div className="mx-auto w-64 md:w-72 lg:w-80 bg-gray-400/20 rounded-2xl shadow-lg px-4 py-1 flex justify-between gap-2">
        {navItems.map(item => (
          <button
            key={item.id}
            id={`bt${item.label}`}
            className={`relative p-2 rounded-md transition group cursor-pointer ${activeNav === item.id ? 'text-black' : 'text-white'}`}
            onClick={() => onNavClick(item.id)}
          >
            {item.icon}
            <span className="absolute animate__animated animate__fadeInDown animate__faster left-1/2 -translate-x-1/2 top-full mt-2 text-xs bg-gray-400/20 text-white px-2 py-0.5 rounded hidden group-hover:block">{item.label}</span>
            <span className={`absolute inset-0 bg-amber-400 transition-transform duration-300 rounded-md -z-10 ${activeNav === item.id ? 'scale-100' : 'scale-0'} group-hover:scale-100`}></span>
          </button>
        ))}
      </div>
    </nav>
